...
def build_battle_outline(llm,system,project,sources,research,cp,log,cancel):
    ...
    if catalog_path.exists():
        ...
        prompt=context+'''\nProduci SOLO il catalogo, con scenes=[]. Due fazioni principali. Identifica i luoghi e i comandanti necessari per raccontare contesto, terreno, attacchi e ritirate; evita duplicati. Includi anche le città di partenza, le tappe della campagna e le località di arrivo che saranno nominate nei titoli o negli eventi: una scena non deve raccontare Ulm, Vienna o un altro luogo mostrando soltanto il campo di battaglia finale. ID tecnici brevi e stabili: soltanto lettere inglesi minuscole a-z, numeri, trattino o underscore; niente accenti, spazi o caratteri locali. Nomi e testi conservano la grafia storica. Riusa esattamente questi ID nelle scene successive.
...
Mostra movimenti delle forze quando pertinenti con routes=[{{side:"a" o "b",points:[[lon,lat],...],uncertain:true}}]; non limitare una battaglia a carte immobili. Una campagna, marcia, conquista, avanzata, ritirata o arrivo richiede almeno un movimento. Il titolo e l'evento non devono nominare una località assente dal catalogo fisso. Traiettorie illustrative coerenti con le località e il terreno, non coordinate tattiche spacciate per certe. Apertura e conclusione hanno focus d'insieme. Eventi simultanei restano simultanei. Non ripetere le scene precedenti. source_ids usa solo fonti consultate; [] se non ci sono riscontri e la modalità è ibrida.
