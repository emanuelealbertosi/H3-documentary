...
def arrange(timeline):
    ...
    for s in timeline["scenes"]:
        ...
        changes.append({"scene":s["id"],"offsets":offsets,"units":[{"id":u['id'],"label_offset":u.get('label_offset',[0,-32]),"screen_offset":u.get('screen_offset',[0,0])} for u in s.get('units',[])]})
    return changes
if __name__=="__main__":
    ...
    for s in p["scenes"]:
        ...
        for unit in s.get('units',[]):
            if unit['id'] in by_id:unit.update({k:by_id[unit['id']][k] for k in ('label_offset','screen_offset')})
