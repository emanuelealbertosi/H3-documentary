...
def issue(batch,expected,target):
...
def request_rows(llm,system,outline,rows,sources,target,log,attempts):
...
SCENE:\n'''+json.dumps(rows,ensure_ascii=False)+'\nFONTI:\n'+local
    ...
    for attempt in range(attempts):
        try:batch=llm.structured(system,prompt,NarrationBatch)
            ...
            last=issue(batch,expected,target)
...
def build_narration(llm,system,outline,project,sources,cp,log,cancel):
    ...
    result=[];count=len(outline['scenes']);target=round(project['minutes']*170/count)
    for first in range(0,count,3):
        ...
        if group_path.exists():batch=read_json(group_path)
            ...
            try:batch=request_rows(llm,system,outline,rows,sources,target,log,2)
            except ModelError as error:
            ...
            if batch is None:
                ...
                for i,row in zip(range(first,last),rows):
                    ...
                    if single_path.exists():single=read_json(single_path)
                        ...
                        single=request_rows(llm,system,outline,[row],sources,target,log,3)
            ...
            problem=issue(batch,list(range(first,last)),target)
