"""Local TTS choices and immutable one-shot voice references."""
import hashlib,io,re,shutil,time,wave
...
def status(cfg):
    return {'default_engine':cfg.get('tts_engine','kokoro'),'default_reference_id':cfg.get('tts_reference_id',''),
            'chatterbox_installed':chatterbox_installed(cfg['pipeline_path']),'voices':voices(),
            'engines':[{'id':'kokoro','name':'Kokoro · veloce, voce italiana if_sara'},
                       {'id':'chatterbox','name':'Chatterbox Multilingual V3 · cloning one-shot'}]}

def ensure_available(engine,reference_id,pipeline_path):
    ...
    if reference_id:voice(reference_id)

def configure_pack(pack,project,work,pipeline_path):
    engine=project.get('tts_engine') or 'kokoro';reference_id=project.get('tts_reference_id') or ''
    ensure_available(engine,reference_id,pipeline_path)
    if engine=='kokoro':
        ...
        if pack.get('voice_engine') not in (None,'kokoro') or 'voice' in pack:
            ...
            pack.pop('voice_reference',None);pack.pop('voice_language',None)
        return pack
    speaker='included';reference_path=''
    if reference_id:
        ...
        reference_path=destination.relative_to(work).as_posix();speaker='clone-'+record['sha256'][:16]
...
def change_project_voice(pid,choice):
    ...
    cfg=store.settings();ensure_available(choice.tts_engine,choice.tts_reference_id,cfg['pipeline_path'])
    ...
    store.update(pid,tts_engine=choice.tts_engine,tts_reference_id=choice.tts_reference_id,status='cancelled',stage='Voce da rigenerare',error='La nuova voce sarà applicata alla ripresa.')
    store.event(pid,'Voce selezionata: '+('Chatterbox Multilingual V3' if choice.tts_engine=='chatterbox' else 'Kokoro if_sara')+'.')
