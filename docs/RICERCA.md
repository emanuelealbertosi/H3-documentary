...
La 1.1.5 separa anche l'autore battle in un catalogo e piccoli gruppi di scene. Questo evita di richiedere e correggere ogni volta un intero piano lungo. In `focus` sono ammessi soltanto gli ID dei luoghi del catalogo; “attacco francese”, “linea difensiva” o “ritorno di Napoleone” sono eventi o descrizioni, non luoghi. `commander_ids` contiene soltanto ID di comandanti. Ogni errore elenca i valori ricevuti e quelli ammessi. I checkpoint `battle-catalog.json` e `battle-progress.json` permettono la ripresa dopo troncamenti o disconnessioni.

La regia 1.1.5 rifiuta prima della voce mappe vuote, movimenti nascosti nelle slide e componenti visuali privi dei dati necessari. Un viaggio usa rotte geografiche documentabili oppure una sequenza animata di tappe dichiarata non geografica; non inventa coordinate per il mito. I personaggi collegati compaiono in riquadri automatici e gli zoom ravvicinati richiedono patch di terreno dettagliate. Una tradizione letteraria mostra l'ordine del racconto senza far scorrere anni fittizi. Pack precedenti senza questa regia conservano la resa precedente.

La logica usa lo stesso client Chat Completions per provider locali e remoti. Le prove remote impiegano server simulati e non chiamano API a pagamento; la prova opzionale locale utilizza soltanto `localhost`.
