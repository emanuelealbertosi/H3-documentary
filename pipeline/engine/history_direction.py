...
Per ogni gruppo di scene conserva continuità delle tappe, evitando episodi duplicati. La narrazione deve raccontare esattamente ciò che queste scene permettono di vedere.'''
    ...
    text+='\nPer ogni visual_layer kind=territory aggiungi boundary_query={"name":"nome storico esatto in inglese","wikidata_id":"Q... solo se noto"}. Il recupero cartografico userà identità ed epoca per cercare geometrie pubblicate; non inventare ID o URL di dataset. Puoi lasciare polygons=[] se non hai coordinate documentate: non disegnare un confine per riempire la scena. Per stati distinti (per esempio Repubblica romana e Impero romano) usa identità separate. influence, alliance, contested e aree culturali/religiose mantengono la propria semantica: non sono confini nazionali.'
    return text
