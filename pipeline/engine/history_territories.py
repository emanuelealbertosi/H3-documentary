...
def scene_area_points(document,scene):
    ...
    for layer in selected_layers(document,scene):
        states=sorted(layer.get('states',[]),key=lambda row:year_value(row['year']))
        prior=[row for row in states if year_value(row['year'])<=start]
        relevant=([prior[-1]] if prior else [])+[row for row in states if start<year_value(row['year'])<=end]
...
def area_view(points):
    ...
    if view[2]>170:raise ValueError('Suddividere i territori in scene regionali: area troppo ampia per questa carta.')
...
def area_style(layer,state):
    ...
    schematic=layer.get('schematic',True)
    ...
    return {'kind':kind,'fill':45 if soft else 72 if kind=='alliance' else 92,
            ...
            'boundary':'area indicativa' if schematic else 'confine documentato'}


def state_blend(layer,year):
    """Layers at absolute time; smooth frame-level opacity, including appearance/loss."""
    now=year_value(year);states=sorted(layer.get('states',[]),key=lambda row:year_value(row['year']))
    active=[row for row in states if year_value(row['year'])<=now]
    if not active:return []
    current=active[-1];years=layer.get('transition_years',0)
    if not years:return [(current,1)]
    q=max(0.,min(1.,(now-year_value(current['year']))/years));fade=q*q*q*(q*(q*6-15)+10)
