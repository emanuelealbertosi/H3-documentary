  ...
  async function upload(files,binding){
    ...
    for(let i=0;i<list.length;i++){
      ...
      try{
        ...
        if(binding){m.bindings=[{...binding,aliases:binding.aliases||[]}];await persist(m);await activateTarget(binding);}
  ...
  function draw(){
    ...
    root.innerHTML=`<div class="topline"><span class="eyebrow">IL TUO ARCHIVIO VISIVO</span><a class="text-link" href="${projectId?'/projects/'+projectId:'/'}">${projectId?'← Torna al progetto':'Crea un documentario →'}</a></div>
      ...
      <div class="media-workspace"><section class="card media-targets"><span class="eyebrow">02 · COLLEGA</span><h2>A chi o a cosa?</h2><p class="tiny muted">Un’immagine può avere più collegamenti. Aggiungi un nome, poi trascinaci sopra l’immagine.</p>
        ...
        <div class="media-target-list">${ts.map((b,i)=>`<div class="media-target-row"><button class="media-target ${m?.bindings.some(x=>x.kind===b.kind&&x.label===b.label)?'linked':''} ${b.visual_state?'visual-'+b.visual_state:''}" data-target="${i}" aria-label="Collega a ${esc(b.label)}"><span>${icons[b.kind]||'◈'}</span><div><small>${b.visual_slot_id?(b.optional?'SUGGERITA':'OBBLIGATORIA')+' · ':''}${esc(kinds[b.kind]||b.kind)}${b.visual_state?' · '+({blank:'PLACEHOLDER',missing:'DA COMPLETARE',empty:'SFONDO FACOLTATIVO',disabled:'ESCLUSA',available:'TROVATA',user:'PERSONALIZZATA'}[b.visual_state]||'NEL FILM'):''}</small><strong>${esc(b.label)}</strong>${b.scene_ids?.length?`<em>${b.scene_ids.length} ${b.scene_ids.length===1?'scena':'scene'}</em>`:''}</div><i>+</i></button>${b.visual_slot_id&&b.visual_editable?`<button class="visual-slot-toggle ${b.enabled?'enabled':''}" data-visual-toggle="${esc(b.visual_slot_id)}" data-visual-enabled="${b.enabled?'true':'false'}">${b.enabled?'Escludi':b.optional?'Attiva suggerimento':'Ripristina'}</button>`:''}</div>`).join('')||'<div class="media-empty">Crea il primo collegamento qui sopra.</div>'}</div>
