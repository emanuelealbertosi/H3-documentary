...
export async function mediaPage({api,esc,toast,projectId}) {
  ...
  function bind(mid,b){const m=items.find(x=>x.id===mid);if(!m)return;
    selected=mid;m.enabled=true;
    if(!m.bindings.some(x=>x.kind===b.kind&&x.label.toLowerCase()===b.label.toLowerCase()))m.bindings.push({...b,aliases:b.aliases||[]});
    persist(m,true);toast('Immagine collegata a '+b.label+'.');
  }
  ...
  function draw(){
    ...
    root.innerHTML=`<div class="topline"><span class="eyebrow">IL TUO ARCHIVIO VISIVO</span><a class="text-link" href="${projectId?'/projects/'+projectId:'/'}">${projectId?'← Torna al progetto':'Crea un documentario →'}</a></div>
      ...
      ${project?`<section class="media-project-note"><label class="check"><input id="project-use-media" type="checkbox" ${project.enabled?'checked':''} ${project.frozen?'disabled':''}> Usa le immagini associate in questo progetto</label><p class="tiny muted">${project.frozen?'Le immagini di questa produzione sono fissate. Le modifiche qui sotto saranno utilizzate nelle prossime produzioni.':'Le associazioni sono riutilizzabili anche negli altri documentari. Salvale prima di avviare la produzione.'}</p></section>`:''}
      ...
      <div class="media-workspace"><section class="card media-targets"><span class="eyebrow">02 · COLLEGA</span><h2>A chi o a cosa?</h2><p class="tiny muted">Un’immagine può avere più collegamenti. Aggiungi un nome, poi trascinaci sopra l’immagine.</p>
        ...
        <div class="media-target-list">${ts.map((b,i)=>`<button class="media-target ${m?.bindings.some(x=>x.kind===b.kind&&x.label===b.label)?'linked':''}" data-target="${i}" aria-label="Collega a ${esc(b.label)}"><span>${icons[b.kind]||'◈'}</span><div><small>${esc(kinds[b.kind]||b.kind)}</small><strong>${esc(b.label)}</strong></div><i>+</i></button>`).join('')||'<div class="media-empty">Crea il primo collegamento qui sopra.</div>'}</div>
