...
export function applyRecoveryBackgroundPreview(box,item){
  if(!box||!isRecoveryBackground(item))return false;
  // The recovered scene renderer fits the whole replacement inside its body.
  Object.assign(box.style,{left:(50/1920*100)+'%',top:(170/1080*100)+'%',width:(1820/1920*100)+'%',height:(765/1080*100)+'%',cursor:'default'});
  box.querySelector('img').style.objectFit='contain';box.querySelector('span').style.display='none';
  return true;
}
export async function mediaPage({api,esc,toast,projectId}) {
  ...
  const bindingKey=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLocaleLowerCase().match(/[\p{L}\p{N}]+/gu)?.join(' ')||'';
  const bindingTerms=b=>new Set([b.label,...(b.aliases||[])].map(bindingKey).filter(Boolean));
  const sameBinding=(a,b)=>a.kind===b.kind&&[...bindingTerms(a)].some(term=>bindingTerms(b).has(term));
  ...
  function position(){const m=activeItem(),box=root.querySelector('#inset-box');if(!m||!box)return;
    if(applyRecoveryBackgroundPreview(box,m))return;
  ...
  function draw(){
    if(disposed)return;
    const m=item(),v=visualItem(),shown=activeItem(),ts=targets(),linkBinding=ts[linkTarget],recoveryBackground=isRecoveryBackground(shown);
    root.innerHTML=`<div class="topline"><span class="eyebrow">IL TUO ARCHIVIO VISIVO</span><a class="text-link" href="${projectId?'/projects/'+projectId:'/'}">${projectId?'← Torna al progetto':'Crea un documentario →'}</a></div>
      ...
      <p class="lead">Collega ritratti, luoghi e materiali ai soggetti del racconto. Il collegamento resta nella memoria visuale e viene riutilizzato nei progetti futuri con lo stesso nome o una delle sue varianti.</p>
      ${project?`<section class="media-project-note"><b>Le immagini che colleghi vengono usate automaticamente.</b><p class="tiny muted">${project.visual?.awaiting_review?'La produzione è ferma prima della voce e del rendering. Collega ciò che vuoi usare, poi continua: ricerca, testo e mappe non verranno rifatti.':project.visual?.completed?'Puoi sostituire qualsiasi immagine del film. Premi Rivedi e aggiorna questo video per controllare anche testi e luoghi, poi avvia l’aggiornamento delle scene coinvolte nello stesso progetto.':project.frozen?'Le immagini della produzione in corso sono già fissate. Potrai aggiornare questo video con altre sostituzioni quando il lavoro sarà terminato.':'Collega le immagini desiderate: non serve attivare un’opzione separata.'}</p></section>`:''}
      ${project?.visual?.ready?`<section class="visual-refresh-bar"><div><b>${project.visual.required_count} obbligatori · ${project.visual.suggested_count} suggeriti</b><span>${project.visual.available_count} trovati · ${project.visual.blank_count} placeholder · ${project.visual.disabled_count} esclusi · ${project.visual.change_count} modifiche pronte</span></div>${project.visual.awaiting_review?`<button id="visual-approve" class="primary">Continua produzione</button>`:project.visual.completed?`<button id="visual-refresh" class="primary">Rivedi e aggiorna questo video</button>`:''}</section>`:''}
      <section class="card media-library"><div class="media-section-title"><div><span class="eyebrow">01 · CARICA</span><h2>Le tue immagini <span class="media-count">${items.filter(x=>x.enabled).length}</span></h2></div><label class="button secondary media-file-label" for="media-files">+ Aggiungi immagini</label><input class="visually-hidden" id="media-files" type="file" accept="image/jpeg,image/png,image/webp" multiple></div>
        ...
        <div class="media-library-tools"><label class="visually-hidden" for="media-search">Cerca un’immagine</label><input id="media-search" placeholder="Cerca nelle tue immagini…" value="${esc(filter)}"><span class="tiny muted">La libreria è condivisa fra tutti i progetti. Un’immagine già associata evita una nuova ricerca o un nuovo download.</span></div>
    ...
    const refresh=root.querySelector('#visual-refresh');if(refresh)refresh.onclick=async()=>{refresh.disabled=true;refresh.textContent='Apro la revisione…';try{await saving;await api('/projects/'+projectId+'/final-review',{method:'POST'});toast('Revisione aperta nello stesso progetto. Controlla e premi Aggiorna questo video.');location.href='/projects/'+projectId;}catch(e){toast(e.message);refresh.disabled=false;refresh.textContent='Rivedi e aggiorna questo video';}};
    ...
    const visualReplace=root.querySelector('#visual-replace');if(visualReplace)visualReplace.onchange=()=>upload(visualReplace.files,{kind:v.kind,label:v.label,aliases:v.aliases||[],visual_slot_id:v.id,visual_editable:project.visual.editable,enabled:v.enabled});
